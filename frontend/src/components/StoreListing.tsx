...
const StoreListing: NextPage<{item: Item}> = ({ item }) => {
    return (
        <div className="flex flex-row snap-x items-center justify-start gap-7 text-center text-smi">
            <div className="snap-start inline-block shrink-0">
                Shops
            </div>
            {item && item.item_infos && 
            item.item_infos.map((store: ItemInfo, index: number) => (
              <div key={store.price} className="snap-start flex flex-col place-items-center gap-4">{/*Change this to unique identifier TODO*/}
                <div className="rounded-8xs w-12 h-10
                                bg-darkolivegreen-300 place-items-center"
                     key={index}>
                    <img
                      className="w-8 h-8"
                      alt=""
                      src={"/asgaard-sofa-312@2x.png" /*TODO: change to logo according to id*/}
                    />
                </div>
                ...
                <div className="text-2xs text-darkolivegreen-100 flex">
                  {(item.item_infos && index == findIndexOfCheapestStore(item.item_infos) ? `Cheapest` : null)}
                </div>
