...
const ProductRow: NextPage<{item: Item}> = ({ item }) => {
  ...
  const ClickAbleBasket = (itemId : number) => {
    // Handle the click event here, e.g., navigate to a new page, open a modal, etc.
    //router.push(`/single-product?id=${itemId}`);
    alert(itemId)
  };
  ...
  return (
    <div className="flex flex-row md:flex-col text-center text-lg text-black font-poppins gap-7">
      <div className="overflow-hidden flex flex-row sm:flex-col sm:w-full justify-between md:justify-center sm:justify-center sm:items-center">
        <img
          onClick={() => ClickableProductRow(item.id)}
          className="w-24 h-24 sm:w-78 sm:h-78"
          alt=""
          src={item.picture_link ? item.picture_link.url : "/"}
        />
        <div className="flex flex-col w-1/4 sm:w-[80%] items-center justify-center gap-[12px]">
          <a onClick={() => ClickableProductRow(item.id)} className="[text-decoration:none] text-[inherit] inline-block">
            {item.name}
          </a>
          <svg onClick={() => ClickAbleBasket(item.id)} className="w-6 h-6 text-darkolivegreen-100" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
