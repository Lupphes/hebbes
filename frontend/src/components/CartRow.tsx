...
const CartRow: NextPage<{item: Item, result: LowestHighest, selectedStore: string | undefined}> = ({ item, result, selectedStore}) => {
    ...
    const handleInputChange = (event:  React.FocusEvent<HTMLInputElement>) => {
        ...
        if (newValue && existingItemIndex !== -1 && parseInt(newValue) !== cartItems[existingItemIndex].cartQuantity) {
            ...
            <tr className="items-center">
                ...
                <td className="items-center">
                    <img
                        ...
                        alt=""
                        src={item.item_info[selectedStore] && item.item_info[selectedStore].store_id == 1 ? (AH.src)
                            : item.item_info[selectedStore] && item.item_info[selectedStore].store_id == 2 ? (JMB.src)
                                : "/"}
                    />
                </td>
                {/*price*/}
                <td className="text-black">
                    € {item.item_info[selectedStore] ? item.item_info[selectedStore].price : item.item_info[keysArray[0]].price}
                </td>
                {/*avg price*/}
                <td className="text-gray">
                    € {item.item_info && Math.round(findAveragePrice(item.item_info) * 100) / 100}
                </td>
                {/*quantity*/}
                <td className="">
                    <input
                ...
                {/*subTotal*/}
                <td className="text-black">
                    € {item.item_info[selectedStore] ? item.item_info[selectedStore].price * item.cartQuantity : item.item_info[keysArray[0]].price * item.cartQuantity}
                </td>
                {/*trashcan*/}
